...
module Network.Wai.Handler.Warp.HTTP2 (isHTTP2, http2, mkSource2, leftoverSource2) where
...
http2 :: Connection -> InternalInfo -> SockAddr -> Transport -> S.Settings -> Source2 -> Application -> IO ()
http2 conn ii addr transport settings src app = do
  ...
  where
    ...
    checkPreface = do
        preface <- readSource2 src connectionPrefaceLength
